...
| Is there an official reversed/mono Torah MiTzion logo? | Still **waiting on client**, and no longer only cosmetic: the mark in the repository is the reversed one, pale blue-white for a dark ground, and on paper it all but disappears. The light theme flattens its colour to keep it legible — the shape is carried by the alpha channel, so nothing is lost but the tint. Replace `:root[data-theme="light"] .brand img` when a dark mark arrives. |
...
| Where are the photographs? (client, 2026-09-16: *the map is large and it is not clear how one gets to the photographs*) | **Beside the map, and on every year.** The landing strip of six random thumbnails is gone. A gallery sits beside the map and follows the pointer — hover a community and its photographs appear, click and they stay, leave and the newest in the album come back; on a phone it sits under the map and a tap pins. The community page's bar chart is gone too: every year is a tile with a cover photograph, an empty year a dashed frame asking *were you here?*, and the community's newest photographs follow as a wall. Both read one new function, `tmz_gallery` (migration `20260916010001_gallery.sql`); until it is pushed the gallery shows the old teaser and the tiles have no covers. Four directions were mocked up for the client; these are the two chosen. |
| A light theme? (client, 2026-09-16) | **Yes, as a choice; dark stays the design and the default.** A switch beside the language picker, remembered per browser, applied by `index.html` before the first paint so the dark ground never flashes past. Light is the same composition on paper rather than an inversion: gold is unreadable on a page, so the accent deepens to bronze and the map's night sky becomes a warm sheet with the land in slate. Any colour ever used with an alpha is now an RGB triplet composed at the use site, so one token serves every wash built from it; the map's own colours moved into the sheet, so switching repaints without redrawing. Every text pairing light renders clears 4.5:1, measured on the real composited grounds. Two pairings in the **dark** theme sit at 3.66:1 (the quietest grey on the page ground) — pre-existing, and the client chose to leave them. |
| Does the photograph viewer follow the light theme? | **No, it stays a dark room in both.** A near-white surround washes a faded photograph out and shifts how its colour reads, which is the one thing this archive exists to show. `.lb` re-declares the dark tokens over its own subtree. Client confirmed. |
| How does one reach the next photograph? | **A drag, and everything else agrees with it.** The stage holds a strip of three — previous, current, next — so the neighbour comes in with the finger. The threshold is 30% of the stage. Buttons and arrow keys go through the same slide, so the three are one movement. In Hebrew the whole set mirrors: the next picture lies to the left, the strip fills the other way round, the nav buttons take logical offsets and their chevrons turn with them. The arrow keys already mirrored; the buttons did not, and now do. |
| Does a click on a community's name open it? | **Yes, in one click.** Every name on the map is a real anchor to its community, not only the selected one's, and it answers the pointer exactly as its dot does. The dot keeps its two steps (choose, then enter); the name is the short way, which is what people reach for first. |
